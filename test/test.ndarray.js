...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof dnansumpw.ndarray, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 4', function test( t ) {
	t.strictEqual( dnansumpw.ndarray.length, 4, 'returns expected value' );
	t.end();
});
...
tape( 'the function calculates the sum of strided array elements (ignoring NaN values)', function test( t ) {
	...
	v = dnansumpw.ndarray( x.length, x, 1, 0 );
	...
	x = new Float64Array( [ NaN, -4.0 ] );
	v = dnansumpw.ndarray( x.length, x, 0, 0 );
	t.strictEqual( v, 0.0, 'returns expected value' );
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `0.0`', function test( t ) {
	...
	v = dnansumpw.ndarray( 0, x, 1, 0 );
	t.strictEqual( v, 0.0, 'returns expected value' );

	v = dnansumpw.ndarray( -1, x, 1, 0 );
	t.strictEqual( v, 0.0, 'returns expected value' );
